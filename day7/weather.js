...
// var url = `https://api.openweathermap.org/geo/1.0/reverse?lat=8.70127682931967&lon=77.7235720484989&limit=5&appid=` + access_key;

// console.log(url)

// let location = "Palayamkottai"
// var url =
//     `https://api.weatherapi.com/v1/current.json?key=1c63857a8e0548f9a1a15
// 2750210909&q=` + location + `&aqi=yes`
...
async function getName(latitude, longitude) {
    const access_key = 'fc3a65fc9ebd7761b6cd0379e6cb3775'
    const url = `https://api.openweathermap.org/geo/1.0/reverse?lat=` +
        latitude + `&lon=` + longitude + `&limit=5&appid=` + access_key;
    const res = await fetch(url);
    const data = await res.json();
    const location = data[0].name.split(" ")[0];
    fetchData(location);
...
2750210909&q=` + location + `&aqi=yes`;
    ...
    if (res.status == 200) {
        ...
        const tags = document.getElementsByClassName('reset');
...
function setValues(data) {
    let setter = document.getElementById("temp_c");
    ...
    setter.innerHTML = `&nbsp;` + data.location.region + `, ` +
        data.location.country;
    setter = document.getElementById("feelslike_c");
    setter.innerHTML = `Feels like ` + data.current.feelslike_c +
        `&#176`;
    ...
    setter.innerHTML = data.current.wind_kph + ` Km/h ` +
